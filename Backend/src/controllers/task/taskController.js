...
const getTaskDetailsHandler = asyncHandler(async (req, res, next) => {
  ...
  if (isMember) {
    ...
    const isAssignedToOtherMembers = assignees.some((id) =>
...
const deleteTaskHandler = asyncHandler(async (req, res, next) => {
  ...
  if (task.status === "completed" || task.status === "submitted") {
    throw new ApiError(400, "Completed or submitted tasks cannot be deleted");
  }
...
const updateTaskHandler = asyncHandler(async (req, res, next) => {
  const userId = req.user;
  ...
  // check project and admin
  const { isAdmin } = await checkProjectExistsAndIsAdmin(
    projectId,
    userId,
    true
  );
  ...
  // record changes only if values differ
  ...
  if (deadline && new Date(deadline).getTime() !== task.deadline?.getTime()) {
  ...
  // if no changes, bail out early
  if (Object.keys(changes).length === 0) {
    return res.json(new ApiResponse(200, "No changes detected", task));
  }
...
const addMemberToTaskHandler = asyncHandler(async (req, res, next) => {
  const userId = req.user;
  const { projectId, taskId } = req.params;
  const { membersToBeAdded = [] } = req.body;
  ...
  // check project and isAdmin
  const { project, isAdmin } = await checkProjectExistsAndIsAdmin(projectId, userId, true);
  if (!isAdmin) throw new ApiError(403, "You are not authorized to perform this action.");

  if (!membersToBeAdded.length) {
    return res.json(new ApiResponse(200, {}, "No members provided"));
  }

  const invalidUsers = [];
  const validUsers = [];
  const members = project.members.map((id) => id.toString());

  membersToBeAdded.forEach((id) => {
  ...
  if (!validUsers.length) {
    return res.json(new ApiResponse(400, { invalidUsers }, "No valid users to add"));
  }

  // update task
  const task = await Task.findOneAndUpdate(
    { _id: taskId, project: projectId },
    { $addToSet: { AssignedTo: { $each: validUsers } } }, // prevent duplicates
    { new: true }
  );
  ...
  return res.json(
    new ApiResponse(200, { task, invalidUsers }, "Members added successfully")
  );
});

const removeMemberFromTaskHandler = asyncHandler(async (req, res, next) => {
  const userId = req.user;
  const { projectId, taskId } = req.params;
  const { membersToBeRemoved = [] } = req.body;
  ...
  //  Check project and admin rights
  const { project, isAdmin } = await checkProjectExistsAndIsAdmin(
    projectId,
    userId,
    true
  ...
  //  Separate valid vs invalid users
  const members = project.members.map((id) => id.toString());
  const validUsers = [];
  const invalidUsers = [];

  membersToBeRemoved.forEach((id) => {
    if (members.includes(id.toString())) {
      validUsers.push(id);
    } else {
      invalidUsers.push(id);
  ...
  if (!validUsers.length) {
    return res.json(
      new ApiResponse(400, "No valid users to remove",{ invalidUsers })
    );
  }

  //  Update task (remove valid users only)
  const task = await Task.findOneAndUpdate(
    { _id: taskId, project: projectId },
    { $pull: { AssignedTo: { $in: validUsers } } },
    { new: true }
  );
  ...
  //  Response
  return res.json(
    new ApiResponse(200, { task, removed: validUsers, invalidUsers }, "Members removed successfully")
  );
});
